...
- 甘特图使用自定义绘制时间轴，不再用普通表格单元格模拟；支持任务条、计划/实际进度、今日线、周末背景、逾期标识、父子任务层级和任务 tooltip。
- 顶部操作按“项目 / 任务 / 数据”分组，数据导入导出集中在菜单中。
- 导入网页版 JSON，兼容：
  - workspace JSON
  - 单项目 JSON
...
- 本地版第一版已经覆盖核心项目、任务、日报增删改，但表格内联编辑、权限协作、公开分享仍由网页版承担。
