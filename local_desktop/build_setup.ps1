param(
    [string]$Version = "Project_Manage_LocalV3.5"
)
...
$InstallBat = Join-Path $SetupSrc "install_project_manage_v3.5.bat"
@"
@echo off
setlocal
set "APP_DIR=%LOCALAPPDATA%\Programs\Project_Manage_LocalV3.5"
if not exist "%APP_DIR%" mkdir "%APP_DIR%"
copy /Y "%~dp0$ExeName" "%APP_DIR%\$ExeName" >nul
powershell -NoProfile -ExecutionPolicy Bypass -Command "`$desktop=[Environment]::GetFolderPath('Desktop');`$shortcut=(New-Object -ComObject WScript.Shell).CreateShortcut((Join-Path `$desktop 'Project Manage Local V3.5.lnk'));`$shortcut.TargetPath=(Join-Path `$env:LOCALAPPDATA 'Programs\Project_Manage_LocalV3.5\$ExeName');`$shortcut.WorkingDirectory=(Join-Path `$env:LOCALAPPDATA 'Programs\Project_Manage_LocalV3.5');`$shortcut.IconLocation=`$shortcut.TargetPath;`$shortcut.Save()"
echo Project Manage Local V3.5 installed to "%APP_DIR%".
exit /b 0
...
InstallPrompt=
DisplayLicense=
FinishMessage=Project Manage Local V3.5 installed.
...
FriendlyName="Project Manage Local V3.5 Setup"
AppLaunched="install_project_manage_v3.5.bat"
PostInstallCmd="<None>"
FILE0="$ExeName"
FILE1="install_project_manage_v3.5.bat"
"@
