...
- GUI-level workflow tests in `local_desktop/tests/test_ui_workflows.py` exercise main-window project/task/daily CRUD plus JSON import and JSON/CSV/Excel export actions using PySide in offscreen mode.
- The desktop Gantt chart is a custom `QAbstractScrollArea`/`QPainter` widget, not a table. It draws a fixed task-info column, horizontal date scale, rounded task bars, planned/actual progress, weekend shading, overdue styling, and today's vertical line.
